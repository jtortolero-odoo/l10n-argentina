...
class DatePeriod(models.Model):
    ...
    special = fields.Boolean(
        string="Opening/Closing Period", help="These periods can overlap."
    )
    ...
    fiscalyear_id = fields.Many2one(
        "account.fiscal.year", string="Fiscal Year", required=True, index=True
    )
    ...
    def _check_year_limit(self):
        ...
        for period in self:
            ...
            if (
                year_date_to < period_date_to
                or year_date_to < period_date_from
                or year_date_from > period_date_from
                or year_date_from > period_date_to
            ):
                raise UserError(msg)
    ...
    def _check_affected_models(self, affected_models):
        for record in self:
            ...
            for model in affected_models:
                searched = self.env[model].search([("period_id", "=", record.id)])
            ...
            if search_dict:
                raise ValidationError(
                    ...
                    % (
                        ("\n").join([repr(x.sorted()) for x in search_dict.values()]),
                        record.name,
                        record,
                    )
                )
    ...
    def search_period_on_date(self, p_date):
        ...
        domain = [("date_from", "<=", period_date), ("date_to", ">=", period_date)]
    ...
    def _get_period(self, period_date):
        ...
        domain = [("date_from", "<=", ddate), ("date_to", ">=", ddate)]
        ...
        if not period:
            ...
            msg = (
                _(
                    "There is no period defined for this date: "
