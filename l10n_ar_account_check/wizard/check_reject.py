...
class AccountCheckReject(models.Model):
    ...
    expense_line_ids = fields.One2many(
        comodel_name="check.reject.expense", inverse_name="reject_id", string="Expenses"
    )
    ...
    def action_reject(self):
        ...
        for check in check_objs:
            ...
            config = check_config_obj.search([("company_id", "=", check.company_id.id)])
            if not config:
                raise ValidationError(
                    _(
                        "ERROR! There is no check \
...
class CheckRejectExpense(models.Model):
    ...
    reject_id = fields.Many2one(comodel_name="account.check.reject", string="Reject")
    price = fields.Float(
