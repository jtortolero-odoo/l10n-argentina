...
class WizardCreateCheck(models.Model):
    ...
    bank_account_id = fields.Many2one(
        comodel_name="res.partner.bank", string="Bank", required=True
    )
    start_num = fields.Char(string="Start number of check", size=20, required=True)
    end_num = fields.Char(string="End number of check", size=20, required=True)
    checkbook_num = fields.Char(string="Checkbook number", size=20, required=True)
