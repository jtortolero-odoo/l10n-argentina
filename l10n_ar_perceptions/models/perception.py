...
class PerceptionPerception(models.Model):
    ...
    applicable_location = fields.Selection(
        [("invoice", _("Invoicing Address")), ("shipping", _("Shipping Address"))],
        "Application Address",
        default="shipping",
        ...
        "* The Shipping Source/Destination address matches the State/Province",
    )  # noqa
    ...
    def _get_concepts_from_account(self, account):
        ...
        query = (
            "SELECT c.id FROM account_account a "
        ...
        if res:
            ...
            raise ValidationError(
                ...
                % (account.name, account.code, self.name)
            )
    ...
    def _check_perception_applicable(self, invoice, perception_data):
        ...
        if self.check_sit_iibb:
            ...
            if sit_iibb == multilateral:
                _logger.info(
                    "Partner es Convenio Multilateral, " + "aplica Percepcion %s",
                    self.name,
                )
                return True
        ...
        if self.applicable_state == "source":
            if self.applicable_location == "invoice":
                ...
                if not partner_state:
                    raise ValidationError(
                        ...
                        + _(
                            "There is no State/Province configured "
                            + "for this Company"
                        )
                    )

            elif self.applicable_location == "shipping":
                origin_sales = sale_obj.search([("invoice_ids", "in", invoice.ids)])
                warehouses = origin_sales.mapped("warehouse_id")
                wh_addresses = map(
                    lambda x: x.partner_id.address_get(["delivery"])["delivery"],
                    warehouses,
                )
                partner_state = map(
                    lambda x: partner_obj.browse(x).state_id.id, wh_addresses
                )
            ...
            if not partner_state:
                address_id = invoice.partner_id.address_get(["delivery"])["delivery"]
                partner_state = partner_obj.browse(address_id).state_id.id
                if not partner_state:
                    raise ValidationError(
                        _("Warning!\n")
                        + _(
                            "Partner has no Delivery State configured. "
        ...
        for line in invoice.invoice_line:
            ...
            if not taxapps:
                raise ValidationError(
                    _("Perception Error!\n")
                    + _(
                        "There is no configured a Perception Application (%s) "
                        + "that corresponds to\nActivity: %s \nConcept: %s\n for"
                        + " the Account %s"
                    )
                    % (self.name, activity_name, concept_name, account_id.name)
                )
            ...
            if len(taxapps) > 1:
                raise ValidationError(
                    _("Perception Error!\n")
                    + _(
                        "There is more than one Perception Application (%s) "
    ...
    def compute(self, perception_data, invoice):
        ...
        applicable = self._check_perception_applicable(invoice, perception_data)
        if not applicable:
            return
        ...
        for concept_id, vals in perceptions.iteritems():
            ...
            # con manual=False
            new_perception_lines += (
                self.env["perception.tax.line"]
                .with_context(manual=False)
                .create(perception_line_vals)
            )
...
class PerceptionVatTax(models.Model):
    ...
    application_id = fields.Many2one(
        "perception.tax.application", "Perception Tax Application", required=True
    )
    rate = fields.Float(
        "Rate",
...
class PerceptionConcept(models.Model):
    ...
    no_subject = fields.Boolean(
        "No Subject",
        help="Check this if Perception should not " + "be applied for this Concept",
    )
    account_ids = fields.Many2many(
        "account.account",
        "account_perception_concept_rel",
        "concept_id",
        "account_id",
        "Accounts",
    )
...
class PerceptionTaxApplication(models.Model):
    ...
    tax_minimum = fields.Float(
        "Tax Minimum",
        digits=dp.get_precision("Account"),
        help="Perception is performed when the amount thereof " + "exceeds this value.",
    )  # Monto Minimo
