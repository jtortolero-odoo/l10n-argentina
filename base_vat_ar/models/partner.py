...
class ResDocumentType(models.Model):
    ...
    def get_website_sale_documents(self, mode="billing"):
        cuit = self.sudo().env.ref("base_vat_ar.document_cuit").name
        cuil = self.sudo().env.ref("base_vat_ar.document_cuil").name
        dni = self.sudo().env.ref("base_vat_ar.document_dni").name
        return self.sudo().search([("name", "in", (cuit, cuil, dni))])
...
class ResPartner(models.Model):
    ...
    dst_cuit_id = fields.Many2one("dst_cuit.codes", string="Country CUIT")
    dst_cuit = fields.Boolean(
        string="Enable DST CUIT", related="document_type_id.dst_cuit"
    )
    ...
    def check_vat_string(self, vals):
        ...
        if not vat:
            return False
        ci_extranjera = self.env.ref("base_vat_ar.document_ci_extranjera")
        pasaporte = self.env.ref("base_vat_ar.document_pasaporte")
        if [x for x in self if x.document_type_id not in [ci_extranjera, pasaporte]]:
            vat = vat.replace(".", "").replace("-", "")
            if not vat.isdigit():
                raise ValidationError(_("The Vat only supports numbers."))
            vals["vat"] = vat
        return False
        ...
        for partner in self:
            ...
            if partner.country_id:
                ...
                try:
                    ...
                    vat_country, vat_number = partner.vat[:2].lower(), partner.vat[
                        2:
                    ].replace(" ", "")
            if not self.document_type_id:
                return True
            if (
                partner.document_type_id
                and not partner.document_type_id.sudo().verification_required
            ):
                return True
    ...
    def _commercial_fields(self):
        return super(ResPartner, self)._commercial_fields() + ["document_type_id"]
