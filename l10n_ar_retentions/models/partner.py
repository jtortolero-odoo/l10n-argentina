...
class res_partner_retention(models.Model):
    ...
    retention_id = fields.Many2one("retention.retention", "Retention", required=True)
        ...
        for p_ret in self:
            if not (0 <= p_ret.excluded_percent <= 1):
                raise ValidationError(
                    _("The Excluded Percent Should be a value " + "between 0 and 1")
                )
...
class ResPartnerAdvanceRetention(models.Model):
    ...
    retention_id = fields.Many2one(
        comodel_name="retention.retention", string="Retention", required=True
    )
    concept_id = fields.Many2one(
        comodel_name="retention.concept", string="Concept", required=True
    )
...
class res_partner(models.Model):
    ...
    def _get_retentions_to_apply(self, operation_date):
        ...
        for p_ret in self.retention_ids:
            ...
            if (
                not (not p_ret.ex_date_from and not p_ret.ex_date_to)
                or (p_ret.ex_date_from <= tzd and not p_ret.ex_date_to)
                or (not p_ret.ex_date_from and tzd <= p_ret.ex_date_to)
                or (p_ret.ex_date_from <= tzd <= p_ret.ex_date_to)
            ):
                excluded_percent = False
