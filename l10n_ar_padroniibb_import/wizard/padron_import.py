...
class PadronImport(models.TransientModel):
    ...
    def import_agip_file(self, rar_file_agip):
        ...
        psql_args_list = [
            "psql",
            "--command=\copy temp_import(create_date,from_date,to_date,vat,multilateral,u1,u2,percentage_perception,percentage_retention,group_per,group_ret,name_partner) FROM "
            + txt_path
            + " WITH DELIMITER ';' NULL '' CSV QUOTE E'\b' ENCODING 'latin1'",  # noqa
        ]
        psql_args_list[1:1] = dsn_pg_splitted
        retcode = call(psql_args_list, stderr=STDOUT)
        assert retcode == 0, "Call to psql subprocess copy command returned: " + str(
            retcode
        )
    ...
    def import_arba_file(self, zip_file_arba):
        ...
        for file_name in files_extracted:
            ...
            if "Ret" in file_name:
                ...
                psql_args_list = [
                    "psql",
                    "--command=\copy temp_import(regimen,create_date,from_date,to_date,vat,multilateral,u1,u2,percentage,u3,u4) FROM "
                    + txt_path
                    + " WITH DELIMITER ';' NULL '' ",  # noqa
                ]
                psql_args_list[1:1] = dsn_pg_splitted
                retcode = call(psql_args_list, stderr=STDOUT)
