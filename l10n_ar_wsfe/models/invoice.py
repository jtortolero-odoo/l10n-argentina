...
class AccountInvoice(models.Model):
    ...
    aut_cae = fields.Boolean(
        "Autorizar", default=False, copy=False, help="Pedido de autorizacion a la AFIP"
    )
    cae = fields.Char(
        string="CAE/CAI",
        size=32,
    ...
    cae_due_date = fields.Date(
        "CAE Due Date", required=False, copy=False, help="Fecha de vencimiento del CAE"
    )
    associated_inv_ids = fields.Many2many(
        "account.invoice",
        "account_invoice_associated_rel",
        "invoice_id",
        "refund_debit_id",
    )
    ...
    def refund(self, date_invoice=None, date=None, description=None, journal_id=None):
        ...
        for refund in refunds:
            ...
            if not refund.fiscal_position_id:
                fiscal_position_id = refund.partner_id.property_account_position
    ...
    def _check_fiscal_values(self):
        ...
        if inv.type in ("out_invoice", "out_refund"):
            ...
            if denomination_id not in inv.pos_ar_id.denomination_ids.ids:
                err = _(
                    "Point of sale has not the same " + "denomination as the invoice."
                )
                raise UserError(_("Error!\n") + err)
        ...
        # Si es factura de proveedor
            ...
            # Chequeamos que la posicion fiscal y la denomination_id coincidan
            if inv.fiscal_position_id.denom_supplier_id.id != inv.denomination_id.id:
                err = _(
        ...
        if inv.fiscal_position_id.id != inv.partner_id.property_account_position_id.id:
            err = _(
    ...
    def action_cancel(self):
        for inv in self:
            if inv.aut_cae:
                err = _(
                    "You cannot cancel an Electronic Invoice "
    ...
    def action_number(self):
        ...
        for obj_inv in self:
            ...
            if invtype in ("out_invoice", "out_refund"):
                ...
                if not m:
                    err = _(
                        "The Invoice Number should be the " + "format XXXX[X]-XXXXXXXX"
                    )
                    raise UserError(_("Error\n") + err)
            ...
            # Si son de Proveedor
                ...
                if local:
                    m = re.match(
                        "(^[0-9]{4}|^[0-9]{5})-[0-9]{8}$", obj_inv.internal_number
                    )
        ...
        for invoice in self:
            ...
            for tax in zero_taxes:
                if tax.tax_id.account_id:
                    ...
                    lines_no_taxes = invoice.invoice_line_ids.filtered(
                        lambda x: x.account_id == tax.account_id
                        and tax.tax_id in x.invoice_line_tax_ids
                    )
    ...
    def _validate_electronic_invoices(self, first_number=False):
        ...
        try:
            invoices_approved = ws.send_invoices(self, first_number=first_number)
    ...
    def get_ws_conf(self):
        ...
        if len(list(set(local_list))) != 1:
            err = _(
                "Trying to get the WSFE config for invoices mixed "
        ...
        if len(list(set(local_list))) != 1:
            err = _(
                "Trying to get the WSFE config for invoices that "
        ...
        if len(confs_list) > 1:
            err = (
                _("There is more than one configuration " + "with this POS %s")
                % pos_ar.name
            )
            raise UserError(_("WSFE Error\n") + err)
        ...
        if confs_list:
            ...
            err = _("There is no configuration for this " + "POS %s") % pos_ar.name
            raise UserError(_("WSFE Error\n") + err)
        else:
            confs_obj = False
        return confs_obj
    ...
    def split_number(self):
        ...
        try:
            ...
            raise UserError(
                _("Error!\n")
                + _("Wrong Number Sequence `%s` for invoice id: `%s`" % (numb, self.id))
            )
        return pos, numb
    ...
    def get_currency_code(self):
        ...
        if not currency_code_ids:
            raise UserError(
                _("WSFE Error!\n")
                + _(
                    "Currency has to be configured correctly "
...
class AccountInvoiceTax(models.Model):
    ...
    def hook_compute_invoice_taxes(self, invoice, tax_grouped):
        ...
        return super(AccountInvoiceTax, self).hook_compute_invoice_taxes(
            invoice, tax_grouped
        )
