...
class WsfexShippingPermission(models.Model):
    ...
    dst_country_id = fields.Many2one(
        "wsfex.dst_country.codes", "Dest Country", required=True
    )
...
class wsfex_incoterms_codes(models.Model):
    ...
    incoterms_id = fields.Many2one(
        comodel_name="account.incoterms", string="OpenERP Incoterm"
    )
    wsfex_config_id = fields.Many2one("wsfex.config")
...
class WsfexVoucherTypeCodes(models.Model):
    ...
    voucher_type_id = fields.Many2one(
        comodel_name="wsfe.voucher_type", string="OpenERP Voucher Type"
    )
    wsfex_config_id = fields.Many2one("wsfex.config")
...
class WsfexConfig(models.Model):
    ...
    point_of_sale_ids = fields.Many2many(
        comodel_name="pos.ar",
        relation="pos_ar_wsfex_rel",
        column1="wsfex_config_id",
        column2="pos_ar_id",
        string="Points of Sale",
    )
    wsaa_ticket_id = fields.Many2one(comodel_name="wsaa.ta", string="Ticket Access")
    ...
    dst_cuit_ids = fields.One2many(
        comodel_name="dst_cuit.codes", inverse_name="wsfex_config_id", string="DST CUIT"
    )
    ...
    def get_wsfex_voucher_types(self):
        ...
        for r in res:
            ...
            # ~ Si los codigos estan en la db los modifico
                ...
                vals = {"name": r.Cbte_Ds, "wsfex_config_id": self.id}
                res_c.write(vals)
