...
class WsfeConfig(models.Model):
    ...
    point_of_sale_ids = fields.Many2many(
        "pos.ar", "pos_ar_wsfe_rel", "wsfe_config_id", "pos_ar_id", "Points of Sale"
    )
    vat_tax_ids = fields.One2many(
        "wsfe.tax.codes", "wsfe_config_id", "Taxes", domain=[("from_afip", "=", True)]
    )
    exempt_operations_tax_ids = fields.One2many(
        "wsfe.tax.codes",
        "wsfe_config_id",
        "Taxes of Exempt Operations",
    ...
    services_date_difference = fields.Integer("Services and Products", default=10)
    products_date_difference = fields.Integer("Products", default=5)

    _defaults = {
        "company_id": lambda self, cr, uid, context=None: self.env[
            "res.users"
        ]._get_company(cr, uid, context=context),
    ...
    def ws_auth(self):
        ...
        auth = {"Token": token, "Sign": sign, "Cuit": self.cuit}
    ...
    def get_voucher_info(self, pos, voucher_type, number):
        ...
        if not hasattr(response, "ResultGet"):
            raise UserError(_("Error fetching voucher from AFIP!"))
    ...
    def read_tax(self):
        self.ensure_one()
        if not self.cuit:
            raise UserError(_("Please configure the company VAT before get Taxes!"))
        wsfe_tax_model = self.env["wsfe.tax.codes"]
        ws = self.ws_auth()
        data = {"FEParamGetTiposIva": {}}
        ...
        for inv in invoices:
            ...
            if not fiscal_position_id:
                raise UserError(
                    ...
                    + _(
                        "There is no fiscal position configured for "
                        + "the customer %s"
                    )
                    % inv.partner_id.name
                )
            ...
            date_due = (
                inv.date_due
                and datetime.strptime(inv.date_due, "%Y-%m-%d").strftime("%Y%m%d")
...
class WsfeVoucherType(models.Model):
    ...
    denomination_id = fields.Many2one(
        "invoice.denomination", "Denomination", required=False
    )
        ...
        if model == "account_invoice":
            ...
            if not len(res):
                raise UserError(
                    ...
                    + _("There is no voucher type that corresponds " + "to this object")
                )
            ...
            if len(res) > 1:
                raise UserError(
                    _("Voucher type error!\n")
                    + _(
                        "There is more than one voucher type that "
