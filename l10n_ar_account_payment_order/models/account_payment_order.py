...
class AccountPaymentOrder(models.Model):
    ...
    narration = fields.Text(string="Notes", default=lambda s: s._get_narration())
    ...
    reference = fields.Char(string="Ref #", default=lambda s: s._get_reference())
    ...
    currency_id = fields.Many2one(
        comodel_name="res.currency", string="Currency", compute="_get_journal_currency"
    )
    amount = fields.Float(
        string="Tax Amount",
        digits=dp.get_precision("Account"),
        default=lambda s: s._get_amount(),
        states={
            "cancel": [("readonly", True)],
            "posted": [("readonly", True)],
        },
    ...
    analytic_id = fields.Many2one(
        comodel_name="account.analytic.account", string="Write-Off Analytic Account"
    )
    ...
    def _get_journal_currency(self):
        for payment in self:
            payment.currency_id = (
                payment.journal_id.currency_id or payment.company_id.currency_id
            )

    @api.model
    def _get_default_company(self):
        company = self.env["res.company"]._company_default_get("account.payment.order")
        return company
    ...
    def _get_journal(self):
        ...
        if not immediate and ttype in ("payment", "receipt"):
            rec = self.env["account.journal"].search(
                [("type", "=", ttype), ("company_id", "=", company.id)],
                limit=1,
                order="priority",
            )
            if not rec:
    ...
    def _paid_amount_in_company_currency(self):
        for p in self:
            ...
            self.paid_amount_in_company_currency = payment.currency_id.compute(
                payment.amount, payment.company_id.currency_id
            )
    ...
    def onchange_partner_id(self):
        if self.journal_id.type in ("sale", "sale_refund"):
            ...
            account_id = (
                ...
                or self.journal_id.default_debit_account_id
            )
    ...
    def update_o2m(self, field_str, lines):
        ...
        new_lines = [
            x
            for x in lines
            if x["move_line_id"] not in old_lines.mapped("move_line_id.id")
        ]
    ...
    def recompute_payment_lines(self, onchange_attr):
        ...
        if line_ids:
            for line in line_ids:
                if line.invoice_id and not line.move_line_id and not line.account_id:
                    the_line = line
        if not the_line:
            return
        line_ids = the_line
        ...
        for line in account_move_lines:
            ...
            if invoice_id:
                ...
                line_residual = currency_obj.compute(
                    company_currency, self.currency_id, abs(line.amount_residual)
                )
        ...
        res = self._prepare_income_debt_lines(
            onchange_attr == "debt_line_ids" and "debt" or "income", account_move_lines
        )

        if res:
            old_lines = getattr(self, onchange_attr)
            setattr(self, onchange_attr, [(0, 0, x) for x in res])
            setattr(self, onchange_attr, getattr(self, onchange_attr) + old_lines)
            setattr(self, onchange_attr, getattr(self, onchange_attr) - line_ids)
        else:
            setattr(self, onchange_attr, False)

    def _compute_writeoff_amount(self, records, amount, type, old_writeoff):
        debit = credit = 0.0
    ...
    def _create_move_line_payment(
        ...
        amount_in_company_currency = self._convert_paid_amount_in_company_currency(
            amount
        )
        debit = credit = 0.0
    ...
    def create_move_lines(self, move_id, company_currency, current_currency):
        ...
        if round(amount_credit, 3) != round(total_credit, 3) or round(
            amount_debit, 3
        ) != round(total_debit, 3):
            if self.type in ("purchase", "payment"):
    ...
    def first_move_line_get(self, move_id, company_currency, current_currency):
        ...
        move_line = {
            ...
            or False,
            "amount_currency": (
                sign * abs(self.amount) if company_currency != current_currency else 0.0
            ),
            "date": self.date,
            "date_maturity": self.date_due,
        }
    ...
    def _get_exchange_lines(
        self, line, move_id, amount_residual, company_currency, current_currency
    ):
        if amount_residual > 0:
            account_id = (
                line.payment_order_id.company_id.expense_currency_exchange_account_id
            )
            if not account_id:
                action_id = self.env.ref("account.action_account_form").id
                msg = _(
                    "You should configure the 'Loss Exchange Rate "
            ...
            account_id = (
                line.payment_order_id.company_id.income_currency_exchange_account_id
            )
            if not account_id:
                action_id = self.env.ref("account.action_account_form").id
                msg = _(
                    "You should configure the 'Gain Exchange Rate "
                    + "Account' to manage automatically the booking of "
                    + "accounting entries related to differences "
                    + "between exchange rates."
                )
    ...
    def voucher_move_line_create(
        ...
        for line in self.line_ids:
            ...
            amount = sign * self._convert_amount(line.untax_amount or line.amount)
            ...
            move_line_analytic_account_id = (
                line.account_analytic_id and line.account_analytic_id.id or False
            )
            move_line = {
                "journal_id": self.journal_id.id,
                "name": line.name or "/",
                ...
                "move_id": move_id,
                "partner_id": self.partner_id.id,
                "currency_id": self.currency_id != company_currency
            ...
            if line.move_line_id:
                ...
                if (
                    ...
                    and line.move_line_id.currency_id != company_currency
                ):
                    ...
                    if line.move_line_id.currency_id == current_currency:
                        ...
                        amount_currency = line.move_line_id.currency_id.compute(
                            move_line["debit"] - move_line["credit"], company_currency
                        )
                if line.amount == line.amount_unreconciled:
            ...
            if (
                ...
                and not line.move_line_id.currency_id.is_zero(foreign_currency_diff)
            ):
    ...
    def writeoff_move_line_get(
        self, line_total, move_id, name, company_currency, current_currency
    ):
        move_line = {}

        current_currency_obj = (
            self.currency_id or self.journal_id.company_id.currency_id
        )
        ...
        if not current_currency_obj.is_zero(line_total):
            ...
            move_line = {
                ...
                "amount_currency": company_currency != current_currency
                and (sign * -1 * self.writeoff_amount)
                or 0.0,
                "currency_id": company_currency != current_currency
                and current_currency.id
                or False,
                "analytic_account_id": self.analytic_id
    ...
    def action_move_line_create(self):
        ...
        for payment in self:
            ...
            line_total, rec_list_ids = self.with_context(ctx).voucher_move_line_create(
...
class AccountPaymentOrderLine(models.Model):
    ...
    payment_order_id = fields.Many2one(
        comodel_name="account.payment.order", string="Payment Order", ondelete="cascade"
    )
    ...
    type = fields.Selection(
        string="Dr/Cr", selection=[("debt", "Debt"), ("income", "Income")]
    )
    ...
    def _compute_writeoff_amount(
        self, debt_line_ids, income_line_ids, amount, type, old_writeoff
    ):
        debit = credit = 0.0
        sign = type == "payment" and -1 or 1
...
class AccountPaymentModeLine(models.Model):
    ...
    name = fields.Text(help="Payment reference", string="Description")
    payment_order_id = fields.Many2one(
    ...
    currency_id = fields.Many2one(
        comodel_name="res.currency", compute="_compute_currency", string="Currency"
    )
    company_currency = fields.Many2one(
        comodel_name="res.currency",
        string="Company Currency",
        readonly=True,
        default=lambda s: s._get_company_currency(),
    )
    date = fields.Date(string="Payment Date", help="This date is informative only.")

    @api.depends("payment_mode_id")
    def _compute_currency(self):
        for i in self:
            i.currency_id = (
                i.payment_mode_id.currency_id or self._get_company_currency()
            )
